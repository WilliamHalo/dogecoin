...
// BIP39 English word list (first 256 words shown, truncated for brevity)
// Complete list must be embedded in source
const char* wordList[] = {
    ...
    "lobster","local","location","lock","locust","lodge","log","lonely","long","loop",
    ...
    "niche","niece","night","nine","noble","noise","nominee","noodle","normal","north",
    ...
    "ocean","october","odor","off","offer","office","officer","official","often","oil",
    ...
    "sail","salad","salmon","salon","salt","salute","same","sample","sand","satellite",
    "satisfy","satoshi","sauce","sausage","save","say","scale","scan","scare","scatter",
    "scene","scheme","school","science","scissors","scorpion","scout","scrap","screen","script",
    "scrub","sea","search","season","seat","second","secret","section","security","seed",
    "seek","segment","select","sell","seminar","senior","sense","sentence","series","service",
    "session","settle","setup","seven","shadow","shaft","shallow","share","shark","sharp",
    "shave","sheet","shelf","shell","shelter","shift","shine","ship","shiver","shock",
    "shoe","shoot","shop","short","shoulder","shove","shrimp","shrug","shuffle","shy",
    "sibling","sick","side","siege","sight","sign","silent","silk","silver","similar",
    ...
    "valve","van","vanish","vapor","various","vast","vault","vector","velvet","vendor",
...
std::vector<unsigned char, secure_allocator<unsigned char>> MnemonicToSeed(const std::string& mnemonic, const std::string& passphrase)
{
    ...
    std::vector<unsigned char, secure_allocator<unsigned char>> seed(64);

    int result = PKCS5_PBKDF2_HMAC(mnemonic.c_str(), mnemonic.length(),
...
{
    if (!word || std::strlen(word) == 0) {
        return -1;
    }

    // Binary search through word list (words are sorted)
    int left = 0;
    int right = BIP39_WORD_COUNT - 1;

    while (left <= right) {
        int mid = left + (right - left) / 2;
        int cmp = std::strcmp(word, wordList[mid]);
...
bool ValidateMnemonic(const std::string& mnemonic)
{
    ...
    // Count words and validate each word is in the list
    while (iss >> word) {
        if (GetWordIndex(word.c_str()) < 0) {
            return false;
        }
        wordCount++;
    }
    ...
    // Checksum calculation: checksumLength = entropyLength / 32
    // For simplicity, we just validate word count and existence in list
    // Full checksum validation would require rebuilding entropy and verifying SHA256
...
{
    // Valid entropy lengths: 128, 160, 192, 224, 256 bits (16, 20, 24, 28, 32 bytes)
    size_t entropyLen = entropy.size();
    if (entropyLen != 16 && entropyLen != 20 && entropyLen != 24 && entropyLen != 28 && entropyLen != 32) {
        return "";
    }

    // Calculate checksum: first (entropy length / 32) bits of SHA256(entropy)
    unsigned char hash[SHA256_DIGEST_LENGTH];
    SHA256(entropy.data(), entropyLen, hash);

    int checksumBits = entropyLen / 4; // Same as entropyLen * 8 / 32
    int totalBits = entropyLen * 8 + checksumBits;
    int wordCount = totalBits / 11;

    // Convert entropy+checksum to indices
    std::vector<int> indices;
    int index = 0;
    int bits = 0;

    for (size_t i = 0; i < entropyLen; i++) {
        index = (index << 8) | entropy[i];
        bits += 8;
        while (bits >= 11) {
            indices.push_back((index >> (bits - 11)) & 0x7FF);
            bits -= 11;
            index &= (1 << bits) - 1;
        }
    }
    ...
    // Add checksum bits
    if (checksumBits > 0) {
        int checksum = hash[0] >> (8 - checksumBits);
        index = (index << checksumBits) | checksum;
        bits += checksumBits;
        while (bits >= 11) {
            indices.push_back((index >> (bits - 11)) & 0x7FF);
            bits -= 11;
    ...
    for (size_t i = 0; i < indices.size(); i++) {
        ...
        mnemonic += GetWord(indices[i]);
    }

    return mnemonic;
}

std::string GenerateNewMnemonic(int strength)
{
    if (strength != 128 && strength != 160 && strength != 192 && strength != 224 && strength != 256) {
        return "";
    }

    int entropyLen = strength / 8;
    std::vector<unsigned char> entropy(entropyLen);

    // Generate random entropy using GetStrongRandBytes
    GetStrongRandBytes(entropy.data(), entropyLen);
