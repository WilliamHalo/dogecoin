...
const char* const BIP39_WORDLIST[] = {
    ...
    "copper", "copy", "coral", "core", "corn", "corner", "correct", "cost", "cotton", "couch",
    ...
    "hamster", "hand", "handle", "harbor", "hard", "harvest", "hat", "have", "hawk", "hazard",
    ...
    "photo", "phrase", "physical", "piano", "picnic", "picture", "piece", "pig", "pigeon", "pilot",
    "pin", "pine", "pink", "pipe", "pistol", "pitch", "pizza", "place", "planet", "plastic",
...
bool CMnemonic::Generate(int nEntropyLen, SecureString& strMnemonic)
{
    if (nEntropyLen < BIP39_ENTROPY_LEN_128 || nEntropyLen > BIP39_ENTROPY_LEN_256)
        return false;

    if (nEntropyLen % 4 != 0)
        return false;

    std::vector<unsigned char> entropy(nEntropyLen);
    GetRandBytes(entropy.data(), nEntropyLen);

    int nChecksumBits = GetChecksumBits(nEntropyLen);
    int nWordCount = (nEntropyLen * 8 + nChecksumBits) / 11;

    std::vector<unsigned char> data(nEntropyLen + 1);
    memcpy(data.data(), entropy.data(), nEntropyLen);
    data[nEntropyLen] = CalculateChecksum(entropy.data(), nEntropyLen);
    ...
    for (int i = 0; i < nWordCount; i++) {
        ...
        if (bitIndex < 5) {
            idx = (data[byteIndex] << 3) >> bitIndex;
        } else {
            idx = ((data[byteIndex] << 8) | data[byteIndex + 1]) >> bitIndex;
        }
        idx &= 0x7FF;
...
bool CMnemonic::Validate(const SecureString& strMnemonic)
{
    std::vector<std::string> words;
    if (!Split(strMnemonic, words))
        return false;

    int nWordCount = words.size();
    if (nWordCount != BIP39_MNEMONIC_LENGTH_12 &&
        nWordCount != BIP39_MNEMONIC_LENGTH_15 &&
        nWordCount != BIP39_MNEMONIC_LENGTH_18 &&
        nWordCount != BIP39_MNEMONIC_LENGTH_21 &&
        nWordCount != BIP39_MNEMONIC_LENGTH_24)
        return false;
    ...
    int nEntropyLen = (nWordCount * 11 - GetChecksumBits(
        nWordCount == 12 ? 16 :
        nWordCount == 15 ? 20 :
        nWordCount == 18 ? 24 :
        nWordCount == 21 ? 28 : 32
    ...
    std::vector<unsigned char> data((nWordCount * 11 + 7) / 8);
    for (int i = 0; i < nWordCount; i++) {
        int idx = WordIndex(words[i]);
        if (idx == -1)
            return false;
    ...
    int nChecksumBits = GetChecksumBits(nEntropyLen);
    unsigned char checksum = CalculateChecksum(data.data(), nEntropyLen);
    
    int shift = 8 - nChecksumBits;
    unsigned char expectedChecksum = checksum >> shift;
    unsigned char actualChecksum = data[nEntropyLen] >> shift;

    return expectedChecksum == actualChecksum;
}

bool CMnemonic::ToSeed(const SecureString& strMnemonic, const SecureString& strPassphrase, std::vector<unsigned char>& seedOut)
{
    if (!Validate(strMnemonic))
        return false;

    std::string normalizedMnemonic = strMnemonic.c_str();
    std::transform(normalizedMnemonic.begin(), normalizedMnemonic.end(), normalizedMnemonic.begin(), ::tolower);

    SecureString salt = "mnemonic" + strPassphrase;
    std::string normalizedSalt = salt.c_str();
    std::transform(normalizedSalt.begin(), normalizedSalt.end(), normalizedSalt.begin(), ::tolower);

    seedOut.resize(BIP39_SEED_LEN);
    
    unsigned char saltHash[CSHA512::OUTPUT_SIZE];
    CSHA512().Write((const unsigned char*)normalizedSalt.data(), normalizedSalt.size()).Finalize(saltHash);

    unsigned char derivedKey[BIP39_SEED_LEN];
    unsigned char U1[CSHA512::OUTPUT_SIZE];
    unsigned char T[CSHA512::OUTPUT_SIZE];
    
    CHMAC_SHA512(saltHash, CSHA512::OUTPUT_SIZE)
        .Write((const unsigned char*)normalizedMnemonic.data(), normalizedMnemonic.size())
        .Write((const unsigned char*)"\0\0\0\1", 4)
        .Finalize(U1);
    
    memcpy(T, U1, CSHA512::OUTPUT_SIZE);
    memcpy(derivedKey, T, BIP39_SEED_LEN);

    for (int i = 2; i <= 2048; i++) {
        unsigned char prev[CSHA512::OUTPUT_SIZE];
        memcpy(prev, U1, CSHA512::OUTPUT_SIZE);
        
        CHMAC_SHA512(saltHash, CSHA512::OUTPUT_SIZE)
            .Write(prev, CSHA512::OUTPUT_SIZE)
            .Finalize(U1);
        
        for (int j = 0; j < CSHA512::OUTPUT_SIZE; j++) {
            T[j] ^= U1[j];
        }
    ...
    memcpy(derivedKey, T, BIP39_SEED_LEN);
    seedOut.assign(derivedKey, derivedKey + BIP39_SEED_LEN);

    memory_cleanse(U1, sizeof(U1));
    memory_cleanse(T, sizeof(T));
    memory_cleanse(saltHash, sizeof(saltHash));

    return true;
}

bool CMnemonic::Split(const SecureString& strMnemonic, std::vector<std::string>& words)
{
    if (strMnemonic.empty())
        return false;

    std::stringstream ss(strMnemonic.c_str());
    std::string word;
    while (ss >> word) {
...
SecureString CMnemonic::Join(const std::vector<std::string>& words)
{
    SecureString result;
...
bool CMnemonic::IsValidWord(const std::string& word)
{
    return WordIndex(word) != -1;
}

int CMnemonic::GetChecksumBits(int nEntropyLen)
{
    return nEntropyLen / 4;
}

unsigned char CMnemonic::CalculateChecksum(const unsigned char* entropy, int nEntropyLen)
{
    uint256 hash = Hash(entropy, entropy + nEntropyLen);
    return hash.begin()[0];
}

int CMnemonic::WordIndex(const std::string& word)
{
